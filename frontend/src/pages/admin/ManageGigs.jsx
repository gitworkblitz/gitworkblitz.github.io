import React, { useState, useEffect } from 'react'
import {
  TrashIcon, RocketLaunchIcon, MagnifyingGlassIcon,
  PlusIcon, PencilSquareIcon, XMarkIcon, CheckCircleIcon, PauseCircleIcon
} from '@heroicons/react/24/outline'
import { getAllDocuments, updateDocument, createDocument, deleteDocument } from '../../services/firestoreService'
import { dummyGigs, formatCurrencyINR } from '../../utils/dummyData'
import { TableSkeleton } from '../../components/SkeletonLoader'
import toast from 'react-hot-toast'

export default function ManageGigs() {
  const [gigs, setGigs] = useState([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  // Modals state
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [modalMode, setModalMode] = useState('create') // 'create' or 'edit'
  const [currentGig, setCurrentGig] = useState(null)
  
  // Form state
  const [formData, setFormData] = useState({
    title: '',
    category: 'Development',
    budget: '',
    status: 'active',
  })
  const [saving, setSaving] = useState(false)

  useEffect(() => { loadGigs() }, [])
  const loadGigs = async () => {
    setLoading(true)
    try {
      const data = await getAllDocuments('gigs')
      setGigs(data.length > 0 ? data : dummyGigs)
    } catch { setGigs(dummyGigs) }
    finally { setLoading(false) }
  }

  const toggleStatus = async (gig) => {
    const newStatus = gig.status === 'active' ? 'paused' : 'active'
    try {
      await updateDocument('gigs', gig.id, { status: newStatus })
      setGigs(prev => prev.map(g => g.id === gig.id ? { ...g, status: newStatus } : g))
      toast.success(`Gig ${newStatus}`)
    } catch { toast.error('Failed to update status') }
  }

  const removeGig = async (id) => {
    if (!window.confirm('Remove this gig completely? This cannot be undone.')) return
    try {
      await deleteDocument('gigs', id)
      setGigs(prev => prev.filter(g => g.id !== id))
      toast.success('Gig removed permanently')
    } catch {
      toast.error('Failed to remove gig')
    }
  }

  const openCreateModal = () => {
    setModalMode('create')
    setCurrentGig(null)
    setFormData({ title: '', category: 'Development', budget: '', status: 'active' })
    setIsModalOpen(true)
  }

  const openEditModal = (gig) => {
    setModalMode('edit')
    setCurrentGig(gig)
    setFormData({
      title: gig.title || '',
      category: gig.category || 'Development',
      budget: gig.budget || '',
      status: gig.status || 'active',
    })
    setIsModalOpen(true)
  }

  const handleSaveGig = async (e) => {
    e.preventDefault()
    if (!formData.title || !formData.budget) {
      toast.error('Title and Budget are required')
      return
    }
    
    setSaving(true)
    try {
      const payload = {
        ...formData,
        budget: Number(formData.budget)
      }

      if (modalMode === 'create') {
        const newGig = {
          ...payload,
          created_at: new Date().toISOString(),
        }
        const docId = await createDocument('gigs', newGig)
        if (docId) {
          setGigs([{ id: docId, ...newGig }, ...gigs])
          toast.success('Gig created successfully')
        }
      } else {
        const gigId = currentGig.id
        await updateDocument('gigs', gigId, payload)
        setGigs(gigs.map(g => g.id === gigId ? { ...g, ...payload } : g))
        toast.success('Gig updated successfully')
      }
      setIsModalOpen(false)
    } catch (err) {
      toast.error(`Failed to ${modalMode} gig`)
    } finally {
      setSaving(false)
    }
  }

  const filtered = gigs.filter(g =>
    !search || 
    g.title?.toLowerCase().includes(search.toLowerCase()) || 
    g.category?.toLowerCase().includes(search.toLowerCase())
  )

  if (loading) return <div className="p-6 max-w-7xl mx-auto"><TableSkeleton rows={6} /></div>

  return (
    <div className="p-6 max-w-7xl mx-auto">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Manage Gigs</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{gigs.length} total gigs posted</p>
        </div>
        <button onClick={openCreateModal} className="btn-primary flex items-center gap-2 shadow-lg shadow-primary-500/30">
          <PlusIcon className="w-5 h-5" />
          Create New Gig
        </button>
      </div>

      {/* Toolbar */}
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="relative w-full sm:max-w-md">
          <MagnifyingGlassIcon className="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          <input 
            value={search} 
            onChange={e => setSearch(e.target.value)} 
            placeholder="Search by title or category…" 
            className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-primary-500 outline-none transition-all dark:text-white"
          />
        </div>
      </div>

      {/* Table */}
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
              <tr>
                {['Gig Title', 'Category', 'Budget', 'Status', 'Actions'].map(h => (
                  <th key={h} className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {filtered.map(g => (
                <tr key={g.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                  <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white text-sm max-w-[250px] truncate">{g.title}</td>
                  <td className="px-6 py-4">
                    <span className="text-xs font-medium bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 px-2.5 py-1 rounded-full whitespace-nowrap">{g.category}</span>
                  </td>
                  <td className="px-6 py-4 text-sm font-bold text-gray-900 dark:text-white whitespace-nowrap">{formatCurrencyINR(g.budget || 0)}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${g.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}`}>
                      {g.status === 'active' ? <CheckCircleIcon className="w-3.5 h-3.5" /> : <PauseCircleIcon className="w-3.5 h-3.5" />}
                      <span className="capitalize">{g.status || 'active'}</span>
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEditModal(g)} className="p-1.5 text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 bg-gray-50 hover:bg-primary-50 dark:bg-gray-800 dark:hover:bg-primary-900/30 rounded-lg transition-colors" title="Edit Gig">
                        <PencilSquareIcon className="w-4 h-4" />
                      </button>
                      <button onClick={() => toggleStatus(g)} className={`p-1.5 rounded-lg transition-colors ${g.status === 'active' ? 'text-amber-600 bg-amber-50 hover:bg-amber-100 dark:bg-amber-900/20 dark:hover:bg-amber-900/40' : 'text-green-600 bg-green-50 hover:bg-green-100 dark:bg-green-900/20 dark:hover:bg-green-900/40'}`} title={g.status === 'active' ? 'Pause' : 'Activate'}>
                        {g.status === 'active' ? <PauseCircleIcon className="w-4 h-4" /> : <CheckCircleIcon className="w-4 h-4" />}
                      </button>
                      <button onClick={() => removeGig(g.id)} className="p-1.5 text-red-400 hover:text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="Delete Gig">
                        <TrashIcon className="w-4 h-4" />
                      </button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {filtered.length === 0 && (
            <div className="text-center py-16 text-gray-400">
              <RocketLaunchIcon className="w-12 h-12 mx-auto mb-3 opacity-50" />
              <p className="text-sm font-medium">No gigs found matching "{search}"</p>
            </div>
          )}
        </div>
      </div>

      {/* Create / Edit Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-5 border-b border-gray-100 dark:border-gray-800 shrink-0">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                {modalMode === 'create' ? 'Create New Gig' : 'Edit Gig Details'}
              </h3>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
                <XMarkIcon className="w-5 h-5" />
              </button>
            </div>
            
            <div className="p-5 overflow-y-auto">
              <form id="gig-form" onSubmit={handleSaveGig} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Gig Title</label>
                  <input 
                    type="text" 
                    value={formData.title} 
                    onChange={e => setFormData({ ...formData, title: e.target.value })} 
                    className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:text-white transition-all"
                    placeholder="e.g. Build a Landing Page"
                    required
                  />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Category</label>
                    <select 
                      value={formData.category} 
                      onChange={e => setFormData({ ...formData, category: e.target.value })} 
                      className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:text-white transition-all"
                    >
                      <option value="Development">Development</option>
                      <option value="Design">Design</option>
                      <option value="Writing">Writing</option>
                      <option value="Marketing">Marketing</option>
                      <option value="Video/Audio">Video/Audio</option>
                      <option value="Other">Other</option>
                    </select>
                  </div>
                  
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Budget (₹)</label>
                    <input 
                      type="number" 
                      value={formData.budget} 
                      onChange={e => setFormData({ ...formData, budget: e.target.value })} 
                      className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:text-white transition-all"
                      placeholder="e.g. 5000"
                      required
                      min="0"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Status</label>
                  <select 
                    value={formData.status} 
                    onChange={e => setFormData({ ...formData, status: e.target.value })} 
                    className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:text-white transition-all"
                  >
                    <option value="active">Active</option>
                    <option value="paused">Paused</option>
                    <option value="completed">Completed</option>
                  </select>
                </div>
              </form>
            </div>

            <div className="p-5 border-t border-gray-100 dark:border-gray-800 shrink-0 bg-gray-50 dark:bg-gray-900 flex gap-3 justify-end rounded-b-2xl">
              <button 
                type="button" 
                onClick={() => setIsModalOpen(false)} 
                className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button 
                type="submit" 
                form="gig-form"
                disabled={saving}
                className="px-6 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-xl transition-colors flex items-center justify-center min-w-[120px] disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
              >
                {saving ? (
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                ) : (
                  modalMode === 'create' ? 'Create Gig' : 'Save Changes'
                )}
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  )
}
